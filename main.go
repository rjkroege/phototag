...
func convertFormat(recs []LocationSample) []LocationValue {
...
// TODO(rjk): add support daterange

func main() {
	log.Println("hello")

	// insert options here as needed
	flag.Parse()
	...
	for _, f := range flag.Args() {
		...
		locations := convertFormat(recs)

		for i := 0; i < 10 && i < len(locations); i++ {
			log.Printf("[%d] %s %f %f" , i , locations[i].timestamp.String(), locations[i].latitude, locations[i].longitude)
		}
		...
		if err := convertToCsv(ofd, locations); err != nil {
...
func convertToCsv(ofd io.Writer, locs []LocationValue) error {
